...
5. **Cron + tick > daemon.** Long-running != resident process. A tick that reads state, makes one transition, writes state, exits, is easier to reason about, debug, kill, and resume than any long-lived process.
