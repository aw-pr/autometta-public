#!/usr/bin/env bash
# op-refs.local.sh: GITIGNORED override for the real 1Password references.
#
# ============================================================================
# This file does NOT live in the repo. The real file lives at:
#
#   ~/.config/autometta/op-refs.local.sh
#
# Set it up once per machine (covers every subscribed repo at once):
#
#   mkdir -p ~/.config/autometta
#   cp templates/op-refs.local.sh.tpl ~/.config/autometta/op-refs.local.sh
#   chmod 600 ~/.config/autometta/op-refs.local.sh
#   $EDITOR ~/.config/autometta/op-refs.local.sh   # paste in your op:// refs
#
# Why not in-repo? The brew-installed CLI runs from a Cellar snapshot
# (/opt/homebrew/Cellar/autometta/<sha>/libexec/), which is a copy of the
# repo at install time, so it cannot see <your-dev-repo>/op-refs.local.sh.
# XDG (~/.config/autometta/) is the one location both the dev checkout and
# the brew-installed CLI can read. It also avoids duplicating refs across
# every subscribed repo on the same machine.
#
# `autometta auth status` prints which file it actually loaded so you can
# always verify which path is in effect.
# ============================================================================
#
# Resolution order (first existing file wins):
#   1. $AUTOMETTA_LOCAL_REFS  (explicit env var override, rarely needed)
#   2. ~/.config/autometta/op-refs.local.sh  (XDG, recommended: use this)
#   3. <repo-root>/op-refs.local.sh  (next to op-refs.sh; dev checkout only,
#      not visible to the brew-installed CLI; useful for one-off testing)
#
# Find a reference in 1Password desktop: right-click an item -> Copy Secret
# Reference. Or via the CLI:
#   op item get "<ItemName>" --vault "<VaultName>" --format=json | jq '.fields[].label'

export OP_REF_OPENAI_API_KEY="op://YOUR_VAULT/OpenAI/credential"
export OP_REF_ANTHROPIC_API_KEY="op://YOUR_VAULT/Anthropic/credential"

# Leave commented unless you store the Claude OAuth token in 1Password as well.
# This is the env-var route for Claude Pro / Max OAuth via op-fetch; for the
# default file-based session at ~/.config/claude/ it is not needed.
# export OP_REF_CLAUDE_CODE_OAUTH_TOKEN="op://YOUR_VAULT/Claude-OAuth/credential"

# ---------------------------------------------------------------------------
# Codex api mode also needs a sibling CODEX_HOME with auth_mode: "apikey",
# because codex prefers $CODEX_HOME/auth.json over the OPENAI_API_KEY env
# var (lessons.md gotcha #8). One-time host setup, after this file has its
# real OPENAI_API_KEY ref above:
#
#   mkdir -p ~/.codex-api-only && chmod 700 ~/.codex-api-only
#   op-fetch --print "$OP_REF_OPENAI_API_KEY" | \
#     CODEX_HOME=~/.codex-api-only codex login --with-api-key
#
# Override the path globally with $AUTOMETTA_CODEX_HOME. The autometta
# spawn scripts and `autometta auth check codex` both verify the sibling
# is initialised and fail closed otherwise.
# ---------------------------------------------------------------------------
