...
main() {
  ...
  local auth_pairs
  if ! auth_pairs="$(REPO_ROOT="$repo_root" "$script_dir/auth-route.sh" "$family" --role worker)"; then
  ...
  local codex_mode=""
  if [[ "$family" == "codex" ]]; then
    if ! codex_mode="$(REPO_ROOT="$repo_root" "$script_dir/auth-route.sh" codex --print-mode --role worker)"; then
